...
export default function Dashboard({ nombres,user }) {
  ...
  useEffect(() => { 
    setOpenProdut(images[currentIndex]);
  }, [currentIndex]);
  ...
  const goToNextImage = (e) => {
    e.stopPropagation();
    setCurrentIndex((nextIndex) => (nextIndex === images.length - 1 ? images.length -1 : nextIndex + 1));
  };
  ...
  return (
    <Layout carrito={carrito} user={user} setNewCarrito={setNewCarrito} eliminar={showEliminar}  mostrar={() => setShowEliminar(!showEliminar)} >
      ...
      <div>
        ...
        galeria ={
          <>
          <button onClick={goToPreviousImage}    className="sticky left-0 top-1/2 ms-2 z-50 bg-blue-500 text-white p-1 rounded-md hover:bg-blue-700 cursor-pointer">anterior</button>
          <button onClick={goToNextImage}     className="sticky left-full top-1/2 me-2 z-50 bg-blue-500 text-white p-1 rounded-md hover:bg-blue-700 cursor-pointer">Sigueite</button>              
        </>
       } 
       >
